...
| CG-013 | `core/third_party_db.py` | Nessuna verifica GPG firme terze parti. | ✅ |
...
- **CG-013:** verifica GPG delle firme di terze parti per i provider che le pubblicano. I feed sanesecurity (junk/phish) pubblicano firme detached `.sig` e vengono verificati con `gpg --verify` (chiave pubblica bundled in `src/core/resources/sanesecurity.gpg`, fingerprint `4E025A1CBA90A0653F38D2D8D691DED931EA4D9E`) PRIMA dell'attivazione: una firma invalida BLOCCA l'aggiornamento (fail-closed, `gpg_required=True`). Gli altri provider (urlhaus, twinclams, ditekshen) non pubblicano firme: per loro resta la mitigazione composita (schema URL, max size, hash SHA-256, test clamscan). Se `gpg` non è installato, la verifica degrada al test clamscan con warning. Pattern `gpg --verify` con `--no-default-keyring` sia all'import che alla verifica (come clamav-unofficial-sigs). Test: `tests/test_third_party_updater.py` (valida/invalida, fail-closed, degrade, regressione). **Backlog Flatpak:** `gpg` non è garantito nel runtime `org.gnome.Platform`; per un deployment robusto andrebbe bundlato `gpg`/`python-gnupg` nel manifest (attualmente si usa `flatpak-spawn --host gpg`).
