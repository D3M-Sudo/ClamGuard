...
class VirusTotalClient:
    ...
    def lookup_file(self, file_path: str, force_refresh: bool = False) -> Optional[Dict]:
        ...
        try:
            ...
            with sqlite3.connect(self.cache_db) as conn:
                conn.execute(
                    ...
                    (file_hash, json.dumps(result), datetime.now().timestamp(),
                     result["malicious"], result["total"])
                )
            return result
    ...
    def upload_file(self, file_path: str) -> Optional[str]:
        ...
        try:
            with open(file_path, "rb") as f:
                resp = self._client.request("files", files={"file": (os.path.basename(file_path), f.read())}, method="POST")
